/* ============================================================
   ...
   Version: 1.1.0
   ------------------------------------------------------------
   FIX: Table is now monotonic. Raw 70–75 all map to 75–76.
...
const Transmutation = (() => {
  ...
  // NOTE: For raw 70–75, the mapping is smoothed to avoid the
  // non-monotonic dip at raw 71 that existed in v1.0.0.
  const TABLE = {
    100: 100,
    ...
    // Smooth band (formerly non-monotonic):
    75: 76,
    74: 76,
    73: 75,
    72: 75,
  ...
  function computeFinalGrade(wwPercent, ptPercent, exPercent, weights) {
    const w = weights || { ww: 0.25, pt: 0.50, ex: 0.25 };
    const raw = (wwPercent * w.ww) + (ptPercent * w.pt) + (exPercent * w.ex);
    const transmuted = transmute(raw);
    return {
      raw: Math.round(raw * 100) / 100,
      transmuted,
      passing: isPassing(transmuted)
    };
  }
